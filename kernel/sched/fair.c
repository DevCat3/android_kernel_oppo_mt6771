...
#if defined (CONFIG_OPLUS_PREFER_SILVER) && defined (OPLUS_FEATURE_UIFIRST)
// BSP.KERNEL.POWER, 2021/7/12,add trigger for prefer little cluster
...
#if defined (CONFIG_OPLUS_PREFER_SILVER) && defined (OPLUS_FEATURE_UIFIRST)
			if (sysctl_prefer_silver && sysctl_uifirst_enabled) {
...
#endif /* CONFIG_OPLUS_PREFER_SILVER && OPLUS_FEATURE_UIFIRST*/
			/*
...
static int detach_tasks(struct lb_env *env, struct rq_flags *rf)
{
	...
	while (!list_empty(tasks)) {
...
#if defined (CONFIG_OPLUS_PREFER_SILVER) && defined (OPLUS_FEATURE_UIFIRST)
		if (sysctl_prefer_silver && sysctl_uifirst_enabled) {
			if (!test_task_ux(p) && is_max_capacity_cpu(env->dst_cpu)) {
		...
		if (sysctl_uifirst_enabled && test_task_ux(p)) {
#ifdef CONFIG_SCHED_WALT
			if((sysctl_launcher_boost_enabled || sysctl_slide_boost_enabled) && is_heavy_ux_task(p) && test_ux_task_cpu(task_cpu(p)) &&
#else
...
next:
		...
		trace_sched_cpu_skip(p, sysctl_prefer_silver, test_task_ux(p),
			...
			prefer_silver_check_task_util(p),
			prefer_silver_check_cpu_util(env->src_cpu),env->src_cpu);
#endif /* CONFIG_OPLUS_PREFER_SILVER && OPLUS_FEATURE_UIFIRST*/
...
#endif
#if defined (CONFIG_OPLUS_PREFER_SILVER) && defined (OPLUS_FEATURE_UIFIRST)
// BSP.KERNEL.POWER, 2021/7/12, Add for trigger prefer silver cpu
